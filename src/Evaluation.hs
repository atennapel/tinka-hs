...
vinst :: Clos v -> v -> Val
vinst (Clos e e' c) v = eval (v : e, e') c
vinst (Fun f) v = f v
...
vappLevel :: Val -> VLevel -> Val
vappLevel (VAbsLevel _ b) v = vinstLevel b v
vappLevel (VNe h sp) v = VNe h (EAppLevel v : sp)
vappLevel (VGlobal x sp w) v = VGlobal x (EAppLevel v : sp) (vappLevel w v)
...
velim v (EAppLevel a) = vappLevel v a
...
eval e c = case {-trace ("eval (" ++ show (length e) ++ ") " ++ show c)-} c of
  ...
  Pi x i t u1 b u2 -> VPi x i (eval e t) (evalLevel (snd e) u1) (Clos (fst e) b) (evalLevel (snd e) u2)
  Sigma x t u1 b u2 -> VSigma x (eval e t) (evalLevel (snd e) u1) (Clos (fst e) b) (evalLevel (snd e) u2)
  Pair a b -> VPair (eval e a) (eval e b)
  Proj t p -> vproj (eval e t) p
  U l -> VU (evalLevel (snd e) l)
  Let x t v b -> eval (first (eval e v :) e) b
  Meta x -> VMeta x
  AppPruning t p -> vappPruning e (eval e t) p
  AppLevel f a -> vappLevel (eval e f) (evalLevel (snd e) a)
  AbsLevel x b -> VAbsLevel x (Clos e b)
  PiLevel x b -> _
